-- Generate Monthly Coupons Script
-- This script will generate 50 monthly coupons (assigned to top spenders first, then unassigned)

-- 1. First, ensure the function exists with the correct definition
...
DECLARE
  ...
  -- Cursor for top spenders from last month
...
BEGIN
  ...
  FOR spender IN top_spenders LOOP
    -- Check if this user already has a coupon this month
    IF NOT EXISTS (
      SELECT 1 FROM public.coupons 
      WHERE user_id = spender.user_id 
      AND is_monthly_reward = true 
      AND month_year = current_month_year
    ) THEN
      ...
      RAISE NOTICE 'Created coupon % for user % (spent % this month)', 
